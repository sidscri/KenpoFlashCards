...
## v3.1.0.6 (build 11) — 2026-01-27

This release was completed in steps. Documentation stays on **v3.1.0.6 (build 11)** while the step work is in progress.
...
### Step 4 — Update behavior + backups + startup options (v3.1.0.6)
...
### Step 5 — Icon + build script stability + cleaner progress (v3.1.0.6)

#### Changed
- EXE/installer icon source is now centralized under `static\res\webappservericons\`:
  - `Advanced Flashcards WebApp Server Icon.png` (source)
  - `AdvancedFlashcardsWebAppServer.ico` (generated for PyInstaller/Inno Setup)
- Packaging build no longer relocates or renames web app/browser icons; only the EXE/installer icon is updated.

#### Added
- `packaging\logs\`: `2. build_exe.bat` now writes build logs here using timestamped filenames.

#### Improved
- `2. build_exe.bat` progress output is now high-level (stage banners) instead of dumping every PyInstaller line to the console.
- On a successful build, the build window auto-closes; on failure, it stays open and points you to the log.
