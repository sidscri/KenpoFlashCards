...
REM   KenpoFlashcardsWebServer-v<major.minor> v<build>.zip
REM ZIP contents root:
REM   KenpoFlashcardsWebServer\...
REM Reads:
REM   .\KenpoFlashcardsWebServer\version.json
REM Excludes (recursively, guaranteed):
REM   .venv, __pycache__, *.pyc
REM Writes log:
REM   logs\KenpoFlashcardsWebServer.log
REM ============================================================
...
set "LOG=%BASE%\logs\KenpoFlashcardsWebServer.log"
> "%LOG%" echo START %DATE% %TIME%
>>"%LOG%" echo BASE=%BASE%

set "PROJ=KenpoFlashcardsWebServer"
set "SRC=%BASE%\%PROJ%"
