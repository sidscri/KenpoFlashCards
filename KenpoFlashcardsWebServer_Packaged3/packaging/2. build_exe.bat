...
for /f "usebackq tokens=* delims=" %%v in (`powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$p = Join-Path '%PROJ_DIR%' 'version.json'; if(Test-Path $p){ (Get-Content $p -Raw | ConvertFrom-Json).appVersion }" 2^>nul`) do set "APP_VERSION=%%v"

call :ENSURE_DIR "%REPO_LOG_DIR%"
set "REPO_LOG=%REPO_LOG_DIR%\build_exe_%STAMP%_v%APP_VERSION%.log"

REM Always create the log immediately (even if we fail on a preflight check)
> "%REPO_LOG%" (
  echo [%DATE% %TIME%] START build_exe
  echo PC=%COMPUTERNAME% USER=%USERNAME% VERSION=%APP_VERSION%
...
if not exist "%PY_EXE%" (
  ...
  echo [INFO] Virtualenv not found. Creating at "%VENV_DIR%"...>> "%REPO_LOG%"
  call :ENSURE_VENV
  if errorlevel 1 (
    echo [ERROR] Failed to create virtualenv / install base tools.
    echo [ERROR] Failed to create virtualenv / install base tools.>> "%REPO_LOG%"
    goto :FAIL
  )
)
if not exist "%REQFILE%" (
  echo [ERROR] Missing: "%REQFILE%"
  echo [ERROR] Missing: "%REQFILE%">> "%REPO_LOG%"
  goto :FAIL
)
if not exist "%SPECFILE%" (
  echo [ERROR] Missing: "%SPECFILE%"
  echo [ERROR] Missing: "%SPECFILE%">> "%REPO_LOG%"
  goto :FAIL
)
if not exist "%PROJ_DIR%\version.json" (
  echo [ERROR] Missing: "%PROJ_DIR%\version.json"
  echo [ERROR] Missing: "%PROJ_DIR%\version.json">> "%REPO_LOG%"
  goto :FAIL
)
if not exist "%DATA_DIR%\kenpo_words.json" (
  ...
  echo [ERROR] Missing: "%DATA_DIR%\kenpo_words.json">> "%REPO_LOG%"
  echo         (Per Step 5, kenpo_words.json must live in <project>\data\)>> "%REPO_LOG%"
  goto :FAIL
)
