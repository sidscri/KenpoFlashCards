...
- **Packaged Version:** **v3.1.0.6 (build 11)**
...
## What's new in v3.1.0.6 (build 11)

This release was built in **steps** while the v3.1.0 cycle is in-progress. Documentation stays on **v3.1.0.6 (build 11)**, and the items below reflect the completed steps:
...
- **Step 4 (v3.1.0.6)**: Update/install behavior: **local data wins**, packaged data seeds missing files; backups on update + on-demand + auto (keep last 10); tray “Restart” now actually relaunches.
...
**Build notes**
- `.venv`, `build`, and `dist` are created in the repo root (`KenpoFlashcardsWebServer_Packaged\`).
- Build logs are written to `packaging\logs\build_exe_<timestamp>_v<version>.log`.
- The build window closes automatically on success; it pauses only on failure.
