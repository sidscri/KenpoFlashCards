...
def _restart_windows_service_sc(name: str) -> bool:
    """Restart service using sc stop/start (best-effort)."""
    if sys.platform != "win32":
        return False
    try:
        ...
        _log(f"[SERVICE][ERROR] SC restart failed: {e}")
        return False

def _run_elevated(cmdline: str) -> bool:
    """Run a command line elevated (UAC)."""
    if sys.platform != "win32":
        return False
    try:
...
def main():
    ...
    # Restart (Auto): if installed as a Windows Service, restart service; otherwise restart tray app.
    if _service_exists(SERVICE_NAME):
        ok = _restart_windows_service_sc(SERVICE_NAME)
        if ok:
            return
    restart_app_only(_icon, _item)
